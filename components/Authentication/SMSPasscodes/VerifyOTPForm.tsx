...
const VerifyOTPForm = (props: Props) => {
  ...
  const resendCode = async () => {
    const { method_id } = await stytchClient.otps.sms.loginOrCreate(
      "+1" + phoneNumber
    );
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (isValidPasscode()) {
      ...
      try {
        await stytchClient.otps.authenticate(otpInput, methodId, {
    ...
    <div>
      ...
      <form onSubmit={onSubmit}>
        ...
        <div style={styles.resendCodeContainer}>
          ...
          <button
            style={{ ...styles.resendCodeButton, ...styles.resendCodeText }}
            onClick={resendCode}
            type="button"
          >
            Resend code
