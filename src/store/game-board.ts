...
import { checkLevelCleared, startGame } from "./controls";
import { scoredPair } from "./score";
import { subtractTiles } from "./tiles-left";
...
export const clicked =
  (clickedIndex): AppThunk => async (dispatch, getState) => {
...
const solveOrCleanup =
  (currentIndex): AppThunk => async (dispatch, getState) => {
    const { gameBoard } = getState();
...
const tileAllowedForSelection = (index: number, board: TokenTileMap) => {
  ...
  const { row, column, layer } = currentTile;

  // Check if the current tile is blocked by any tile on a higher layer
  const isBlockedByHigherTile = Object.values(board).some((tile) => {
    return (
      tile.layer > layer &&
      tile.row === row &&
      tile.column === column
  ...
  if (isBlockedByHigherTile) {
    // The current tile is blocked by a tile on a higher layer
    return false;
  }

  // Proceed with existing edge case checks and selection logic
